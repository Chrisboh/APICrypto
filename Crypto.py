...
for i in range(len(data)):
    for coin in coins:        
        if data[i]["symbol"] == coin["symbol"]:           
            totalpaid = coin["amount_owned"] * coin["price_per_coin"]
            currentvalue = data[i]["quote"]["USD"]["price"] * coin["amount_owned"]
            profit = currentvalue - totalpaid
            
            pl_percoin  = data[i]["quote"]["USD"]["price"] - coin["price_per_coin"]
            total_pl_coin = pl_percoin * coin["amount_owned"]
       
            print(data[i]["name"] + "--" + data[i]["symbol"])
            
            print("price---{0:.2f}".format(data[i]["quote"]["USD"]["price"]))
